...
func Test_write_slot_index_to_file_head(t *testing.T) {
	...
	defer mgr.Close()
	writer := mgr.newWriter(10, 4)
	defer writer.Close()
	seq, tailSeq, slotIndex0, err := writer.newSlotIndex(0, level0)
	should.Equal(slotIndexSeq(0), seq)
	should.Nil(err)
	seq, tailSeq, slotIndex1, err := writer.newSlotIndex(tailSeq, level1)
	should.Equal(slotIndexSeq(0x4f10), seq)
	should.Nil(err)
	seq, tailSeq, slotIndex2, err := writer.newSlotIndex(tailSeq, level2)
	...
	idx, err := writer.mapWritableSlotIndex(0, level0)
	should.Nil(err)
	should.Equal(uint64(123), idx.children[0])
	should.Equal(biter.Bits(0), idx.pbfs[0][0])
	idx, err = writer.mapWritableSlotIndex(0x137d00, level2)
	should.Nil(err)
	should.Equal(uint64(123), idx.children[0])
	should.Equal(biter.Bits(0), idx.pbfs[0][0])
